...
| [0018](./adr-0018-session-datasource-trait.md) | `SessionDataSource` trait + dual-path semantics (adapter-wins on drift, warning-only; async re-upsert prototype removed, `--quiet` suppresses divergence warnings). Footnote links to ADR-0021 for why OTLP is not an `Adapter` | Accepted (M2.1) | `core::datasource` + `adapters::datasource` + `storage::datasource` + `cli` |
...
| [0023](./adr-0023-cache-metrics.md) | Cache token analytics + render policy — `CacheMetrics` (naive + honest hit-rate / saved-tokens), `CACHE_READ_DISCOUNT = 0.9` + `CACHE_WRITE_PREMIUM = 0.25`, `None`-on-zero-activity, no cache cols for `--by tool` / `--by mcp-server`, snake_case `cache_metrics` JSON field | Accepted (M2.5) | `core::analyzer::cache` + render surfaces (md/html/json/list/aggregate) + `tui::views::models` |
...
| [0026](./adr-0026-report-redaction.md) | Report redaction — core `analyzer::redact` layer + `--privacy <none\|redact\|anonymize>` level semantics (analyze + aggregate; md/json/csv + html full; analyze html/speedscope flamegraph closed by F-10/ADR-0028) | Accepted (L-1) | `core::analyzer::redact` + `cli::cmd::{analyze, aggregate}` |
...
| [0028](./adr-0028-episodes-redaction.md) | Episodes redaction — shared `RedactionContext` + `Episodes::redact_with` (analyze html/speedscope fully redacted) + `list --privacy`; closes ADR-0026 deferred | Accepted (F-10) | `core::analyzer::redact` + `cli::cmd::{analyze, list}` |
