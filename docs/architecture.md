...
aggregate [--agent copilot]                  # ✅ M1.6.2: copilot only
          ...
          [--privacy none|redact|anonymize]  # ✅ L-1 — opt-in report redaction (default none); anonymize writes `agentprof-redaction-map.json` sidecar; md/json/csv/html all fully redacted (aggregate html is table-only, no flamegraph) (see ADR-0026)
...
serve   [OPTIONS]                              # ✅ M2.3 (v0.3.3, feature: web)
    ...
    （sessions / session detail / aggregate / mcp-waste list / mcp-waste
    detail），所有 view 都额外提供 `/api/<view>.html` chunk endpoint 供
    ~80 LOC 的 vanilla JS poller 通过 `#main.innerHTML` swap 实时刷新
    （默认 5 s 间隔，toolbar 内 1..=60 s 或暂停；选择 localStorage 持久化）。
      --storage-path <PATH>     SQLite store 路径（必填，文件必须已存在）
      --bind <ADDR>             监听地址，默认 127.0.0.1:4329（非 loopback 会 warn）
      --interval-default <SEC>  浏览器端默认 poll 间隔（默认 5）
      --no-open                 关闭 "启动时调 xdg-open 拉浏览器" 行为
      --quiet                   抑制启动 banner
    Bundled CSS / JS / favicon 由 `include_str!` / `include_bytes!`
    编译期嵌入 binary（零 runtime FS 读、无 CDN）。
    **要求 store mode** —— `--storage-path` 缺失或路径不存在直接退出
    `ExitKind::UserError` 并给出 `agentprof db ingest` 修复提示
    （ADR-0024 D-5：不做 adapter scan fallback，否则与 poll 模型冲突）。
    ...
    feature index 见 [`docs/features/web-dashboard.md`](features/web-dashboard.md)。
...
| 层 | 错误类型 | 工具 |
|---|---|---|
| lib（core/adapters/storage/tui） | `CoreError`、`AdapterError`、`SqliteError`（+ OTLP 的 `OtlpServerError` 等）、`TuiError`（强类型） | `thiserror` |
...
- 真实日志的脱敏目前靠**手动 `sed` / `jq`** 替换路径、邮箱、token（`xtask anonymize` 工具尚未实现，见 [ADR-0003](internals/adr-0003-synthetic-fixture-strategy.md) + [`docs/features/privacy.md`](features/privacy.md) §5）；新 fixture 优先用 synthetic 数据
...
| **Phase 2 (M2.5)** observability | + cache analytics across `analyze` / `list` / `aggregate` / TUI; closes audit finding F-NEW-2 (write-only `total_cache_*` columns now have a read path) + §18 Q4a (observational cache utilization signal) | observational cache utilization signal — `CacheMetrics` (honest + naive hit-rate, net + gross saved tokens) surfaced on 7 render surfaces; ZERO schema change | ✅ **shipped v0.3.1** — see [ADR-0023](internals/adr-0023-cache-metrics.md) |
| **Phase 2 (M2.3)** web dashboard | + `agentprof serve`（feature `web`）+ axum / tower 栈 | localhost HTTP dashboard，5 个轮询视图（sessions / session / aggregate / mcp-waste），复用 M2.1 SQLite + M2.2 axum | ✅ **shipped v0.3.3** — see [ADR-0024](internals/adr-0024-web-dashboard-architecture.md) |
