...
> - **M1.6.3 ✅ shipped 2026-06-01**: `watch` subcommand (single-session + `watch aggregate ...` cross-session) + `aggregate --export tui` activation (deferred from M1.6.2). New `agentprof_tui::watch::{WatchRunner, WatchData, RefreshKind, ReloadError, AggSortKey}` + `Event::Refresh` + cross-session arm in `views::aggregate`. New direct workspace dep `notify-debouncer-mini = "0.4"` (pulls `notify` v6.1.1 transitively). See [`docs/superpowers/specs/2026-06-01-m1.6.3-watch-and-aggregate-tui-design.md`](../docs/superpowers/specs/2026-06-01-m1.6.3-watch-and-aggregate-tui-design.md) + [ADR-0009](../docs/internals/adr-0009-watch-runner-and-notify.md).
> - **M1.6.4 ✅ shipped 2026-05-31 + 2026-06-02**:
>   - **2026-05-31** — Speedscope JSON + HTML report exporters. See [`docs/superpowers/specs/2026-05-31-m1.6.4-speedscope-and-html-export-design.md`](../docs/superpowers/specs/2026-05-31-m1.6.4-speedscope-and-html-export-design.md) + [ADR-0007](../docs/internals/adr-0007-speedscope-export.md).
>   - **2026-06-02** — Tracing infrastructure (canonical observability across all 5 crates): converts 13 production `eprintln!` calls to `tracing::*!`; adds global `--log-level` / `--log-file` flags (clap `global = true`) + env fallbacks (`AGENTPROF_LOG_LEVEL` / `AGENTPROF_LOG_FILE` / `AGENTPROF_LOG_FULL_PATHS`); TUI auto-redirects to `$XDG_STATE_HOME/agentprof/agentprof.log` (rolling daily via `tracing-appender`); 4-layer span topology `cmd.*` → `adapter.*` → `analyzer.*` / `aggregator.group_by*` → events; PII: session paths default to sha256[..8] hash via `agentprof_core::observability::pii::{hash_path, hash_short}`. New deps: `sha2 = "0.10"` (core only) + `tracing-appender = "0.2"` (cli only); no `deny.toml` change. See [`docs/superpowers/specs/2026-06-02-tracing-design.md`](../docs/superpowers/specs/2026-06-02-tracing-design.md) + [`docs/superpowers/plans/2026-06-02-m1.6.4-tracing.md`](../docs/superpowers/plans/2026-06-02-m1.6.4-tracing.md) + [ADR-0010](../docs/internals/adr-0010-tracing-infrastructure.md).
