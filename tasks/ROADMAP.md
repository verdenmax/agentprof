...
| L-11 | ~~**`xtask audit-pii` 不存在**：fixture/report 脱敏全靠人工 `sed`~~ → **✅ 已交付**（`cargo run -p xtask -- audit-pii <path>`：扫描 `/home//Users//C:\Users\` 真实路径，placeholder `USER`/`<user>`/`<username>` 白名单，命中 exit 2） | ✅ FIXED | [`docs/features/privacy.md`](../docs/features/privacy.md) §5 + [plan](../docs/superpowers/plans/2026-06-29-pii-audit.md) | — |
| L-12 | ~~**CI 无 `/home/<user>/` grep guard**~~ → **✅ 已交付**（`pii-guard` job 每次 PR 跑 `audit-pii crates`，意外 commit 真实路径被自动拦截） | ✅ FIXED | 同上 §5 | — |
