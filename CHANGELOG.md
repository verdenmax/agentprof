...
- `agentprof_core::observability::pii::{hash_path, hash_short}` PII redaction helpers (M1.6.4) — sha256[..8] hex. `hash_path` accepts `&Path` (lossy-UTF-8 string form); `hash_short` accepts arbitrary `&str`. Used by the cli + adapter + core layers to emit `session = %hash_path(p)` in tracing fields. `AGENTPROF_LOG_FULL_PATHS=1` opts out (cli layer only — core and adapters always hash). See [ADR-0010 D-5](docs/internals/adr-0010-tracing-infrastructure.md) for the collision-vs-PII trade-off.
