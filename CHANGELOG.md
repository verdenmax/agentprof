...
- **xtask:** `audit-pii <path>` — scans tracked source for real `/home/<user>/`, `/Users/<user>/`, and `C:\Users\<user>\` paths (placeholders `USER` / `<user>` / `<username>` allowlisted), printing every hit and exiting 2 on any leak. CI `pii-guard` job runs it over `crates/` on every PR, auto-blocking the most common accidental PII leak. (L-11, L-12)
