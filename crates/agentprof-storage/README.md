...
| `otlp` (feature `otlp`, M2.2 in progress) | OTLP receiver subsystem. Submodules: `config` (`OtlpServerConfig` + `PartialOtlpServerConfig`, T2.1), `error` (`OtlpServerError` / `MapperError` / `RouterError`, T2.1), `pipeline` (`IngestPipeline` stub counters, T2.2), `server_grpc` (tonic gRPC listener with 3 collector services, T2.2), `server_http` (axum HTTP/protobuf listener with `/v1/{logs,metrics,traces}`, T3.1), `auth` (bearer-token tonic interceptor + axum middleware applied to both transports, T4.1), `tls` (rustls server config + optional mTLS, T4.2), `typed` (`TypedEvent` IR + `SignalKind` + `TokenDirection`, T5.1), `mapper` (OTLP wire types → `TypedEvent`, T5.2), `router` (`SessionRouter` + `SessionBuffer` + OOM caps + `FlushSink` trait + tool-call pairing in `into_persistable`, T6.1), `sweeper` (`spawn_idle_sweeper` async wrapper that periodically calls `router.sweep_idle()` and drains via `flush_all(Shutdown)` on cancellation, T6.2). Internal `proto` submodule holds tonic-generated server stubs (mirrors `opentelemetry::proto::*` layout). |
