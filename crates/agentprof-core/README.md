...
| Module | Highlights |
|---|---|
| `adapter` | `Adapter` trait, `Event` trait (14 methods: 4 required — `kind` / `id` / `timestamp` / `parent_id` — plus 4 default-`None` payload-* overrides (`payload_name` / `payload_model` / `payload_output_tokens` / `payload_mode`), 1 default-empty-`Vec` override `payload_tool_requests` (adapter-supplied `(tool_call_id, arguments)` pairs), 1 default-`None` override `tool_call_id` (used by `derive_episodes` PASS 0 args-lookup), 1 default-`None` override `payload_model_metrics` (per-model `BTreeMap<String, ModelUsage>` rollup, F1.7), 2 default-`None` overrides `payload_success` / `payload_error_message` (B1 — wire success bit + error message for `tool.execution_complete` + `hook.end`), and **M2.1 T5.2.5** 1 default-empty-`BTreeSet` override `payload_loaded_mcp_tools` (per-event MCP-filtered tool catalog announce; accumulated by `derive_episodes` into `Episodes::loaded_mcp_tools`); see [ADR-0011](../../docs/internals/adr-0011-turn-detail-and-args-plumbing.md) + [ADR-0012](../../docs/internals/adr-0012-session-model-metrics-and-models-view.md) + [ADR-0013](../../docs/internals/adr-0013-event-success-bit.md) + [ADR-0015](../../docs/internals/adr-0015-mcp-waste-architecture.md)), `EventKind` (29 variants), `AgentKind`, `SessionRef`, `AdapterError` |
...
| `episode` | `derive_episodes<E>`, `Episodes` (含 F1.7 新增的 `model_metrics: Option<BTreeMap<String, ModelUsage>>`，`#[serde(skip_serializing_if = "Option::is_none")]`；由 `derive_episodes` 在 `EventKind::Shutdown` 事件上从 `Event::payload_model_metrics()` 填充，last-wins per ADR-0012 D-6；**M2.1 T5.2.5** 新增 `loaded_mcp_tools: BTreeSet<String>`，由 `derive_episodes` 每事件累积 `Event::payload_loaded_mcp_tools()`，ever-loaded 语义 ADR-0015 D-2，`#[serde(skip_serializing_if = "BTreeSet::is_empty")]`), `Turn`, `ToolEpisode`, `ToolCall` (含 F1 新增的 `arguments: Option<serde_json::Value>`，`#[serde(skip_serializing_if = "Option::is_none")]`；adapter-supplied、由 `derive_episodes` PASS 0 通过 `Event::payload_tool_requests` + `Event::tool_call_id` 配对回填), `HookEpisode`, `SkillEpisode`, `ModeSegment`, `CallRef`, `Mode` (`Interactive` / `Plan` / `Autopilot` / `Unknown(String)`，对齐真实 Copilot wire), `TurnStatus`, `ToolCallStatus`, `DeriveWarning` (5 variants 含 `SynthesizedStart` / `OpenAtEndOfSession` / `AbortWithoutOpenElement` / `NonMonotonicTimestamp` / `PayloadNameMissing`), `pub const ORPHAN_TOOL_SENTINEL = "<orphan>"` |
| `analyzer` | **`analyze(&Episodes, &SessionMeta, &[ParseWarning]) → AnalysisReport`** (3-arg since M1.4 post-output-audit)；`AnalysisReport` 含 `parse_warnings: Vec<ParseWarning>` 字段，F1.7 新增的 `model_metrics: Option<BTreeMap<String, ModelUsage>>`（由 `analyze()` 从 `Episodes.model_metrics` clone，`#[serde(skip_serializing_if = "Option::is_none")]`，自动流向所有 exporter 与 TUI Models view），以及 **M2.1 T5.2.5** 新增的 `loaded_mcp_tools: BTreeSet<String>`（由 `analyze()` 从 `Episodes.loaded_mcp_tools` clone；`#[serde(default, skip_serializing_if = "BTreeSet::is_empty")]` 兼容 pre-M2.1 存储 JSON；让 `aggregate` / `mcp_waste` / `analyze` 等 CLI 路径无需保留原始 `Vec<CopilotEvent>` 即可读到 ever-loaded 集合，解锁 `SessionDataSource` 流水线）。**M2.1 T2.4** 新增 4 个 token 累加 accessor：`AnalysisReport::total_input_tokens()` / `total_output_tokens()` / `total_cache_read()` / `total_cache_creation()` —— 跨 `model_metrics` 各模型 `ModelUsage` 字段做 `u64::saturating_add` 并 clamp 到 `i64::MAX`，`Option<i64>`，None 表示没有 `model_metrics`；`agentprof-storage::upsert_report` 用它们直接落 `sessions.total_*` 列。**M2.5** 新增 `AnalysisReport::cache_metrics() -> Option<CacheMetrics>`（per-session 入口，零 cache 活动返回 `None` —— ADR-0023 D-1 + D-4），由所有 render surface 用 `Option::is_some()` 判断是否输出 Cache 区块。`turn_summary` / `tool_rank` / `hook_rank` rollups；`ToolRankRow.is_user_blocking: bool` + `pub const USER_BLOCKING_TOOLS: &[&str] = &["ask_user"]`；`percentile` helper；`duration_ms` / `duration_ms_opt` serde helpers；`ModelUsage` (4 `u64` token fields + `new()` / `total()` saturating sum, `#[non_exhaustive]`, F1.7 foundation for per-model session totals) |
| `analyzer::cache` | **M2.5** — `CacheMetrics` struct (creation_tokens / read_tokens / hit_pct_honest / hit_pct_naive / saved_tokens_net / saved_tokens_gross) + Anthropic prompt-cache pricing constants `CACHE_READ_DISCOUNT = 0.9` 与 `CACHE_WRITE_PREMIUM = 0.25`（Claude Sonnet 4.x 2026-06 rates）+ 两条 hit-rate 公式（honest = `read / (read + creation)`，naive = `read / (read + input)`）+ 两条 saved-token 公式（net = `read * 0.9 − creation * 0.25`，可负；gross = `read * 0.9`）。`CacheMetrics::from_totals(input, read, creation) -> Option<CacheMetrics>` 在 read + creation 均为 0 时返回 `None`（ADR-0023 D-1：无 cache 活动 → 不渲染空表）。所有公开 surface（per-session `AnalysisReport::cache_metrics()` + per-bucket `AggregateReport::cache_metrics_per_bucket()`）都包裹 `Option`，让 md / html / json / list / aggregate 渲染层用 `if let Some(...)` 干净 short-circuit。详见 [ADR-0023](../../docs/internals/adr-0023-cache-metrics.md)。 |
...
| `analyzer::redact` | **L-1/F-10** — `PrivacyLevel` (None/Redact/Anonymize), `RedactionMap`, `RedactionContext` (shared cross-report accumulator). `AnalysisReport::redact`/`redact_with`, `Episodes::redact_with`, `WasteReport::redact_with`: collapse model→family, rekey UUIDs→`<uuid-N>`, hash MCP servers, zero timestamps @anonymize. See [ADR-0026](../../docs/internals/adr-0026-report-redaction.md)/[ADR-0028](../../docs/internals/adr-0028-episodes-redaction.md) + `docs/features/privacy.md` |
