...
| `model` | `RawSession<E>` (含 `parse_warnings: Vec<ParseWarning>`), `SessionMeta`, `ToolSource` + `ToolSource::infer`；M1.6.5 新增 `model::waste` 子模块（`WasteReport` / `McpServerWaste` / `McpToolWaste` / `LoadedSource` / `WasteDataSource` / `AggregateWasteReport` / `McpServerCrossWaste` / `McpToolUsageAcrossSessions`，全部 `#[non_exhaustive]` 为 M1.6.6 token-cost 字段扩展预留空间；see [ADR-0015](../../docs/internals/adr-0015-mcp-waste-architecture.md)） |
...
| `analyzer::waste` | M1.6.5 T1.3 新增 — `compute_waste(&AnalysisReport, &BTreeSet<String>, Option<&BTreeMap<String, Vec<String>>>) → WasteReport` 纯函数：以 `report.tool_rank` 推 called 集，与 wire-protocol `tools/list` + mcp.json baseline 合并出 loaded 超集（`LoadedSource::Wire` / `Config` / `Both` / `InferredFromCall` 优先级标注 provenance），按 server 分组，server 内 tools 按 `short_name` 字母序，server 间按 `unused_count` desc / `server` asc 排序；`data_source` 字段总览（`Wire` / `Config` / `Both` / `None`）；`#[tracing::instrument(name = "analyzer.waste", ...)]` per ADR-0010；`aggregate_waste` cross-session reducer 由 T1.4 追加；see spec §6 + [ADR-0015](../../docs/internals/adr-0015-mcp-waste-architecture.md) |
