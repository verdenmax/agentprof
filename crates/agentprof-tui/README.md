...
| `AppRunner::new_with_waste(&AnalysisReport, &Episodes, &WasteReport)` | Waste-aware constructor (M1.6.5) — additive sibling that also threads a `WasteReport` for the `[5] McpWaste` split-pane view |
...
| `views::View` | `Flamegraph` / `Roi` / `Aggregate` / `Models` / `McpWaste` |
...
| `views::flamegraph` | Per-turn horizontal gantt + `segment_layout` + `build_gantt_cells` (3-state row: `█` tool / `░` LLM thinking / `·` padding) + `build_styled_cells_with_source` (colors `█` by [`ToolSource`](../agentprof-core/src/model/tool_source.rs): Builtin=cyan, MCP=magenta, Skill=yellow; reuses `theme::tool_source_color`) + `t_id_status_color(turn, episodes, now)` colors the 5-char T-id span by turn status (F1.10 + F2.2 — precedence: Aborted=Red > **Pending=Yellow (F2.2 — any tool_call in turn is `is_pending(now)`)** > Open=DarkGray > thinking-only=Blue > default=none; aborted still UNDERLINED as color-blind backup) + `is_turn_pending(turn, episodes, now)` helper (F2.2 — iterates `turn.tool_calls`, delegates per-call check to `agentprof_core::analyzer::pending::is_pending`) + 5-char output-tokens column between duration and gantt (via `format_tokens_short`; `None` → centered dash) + `header_line()` sticky column-name header (F1.8 — labels `Turn` / `Duration` / `OutTK` aligned over the data columns + colored legend `█ tool ░ thinking · padding`; rendered in a reserved top strip when `inner.height >= 3`; `OutTK` = per-turn output tokens — input / cache tokens are session-level only, see Models view key `4`) + `selected_turn_footer_line` (footer beneath the gantt listing the selected turn's tool calls with per-call durations, e.g. `T3 selected:  bash(120ms) +2 more`; appends `· thinking only` when the selected turn has no tool calls) + `format_meta_line()` 1-row no-border meta strip below the bordered block (F1.9 — replaces the old 3-row " Detail " block; shows `Tn · +rel in · dur · N calls · model=… · mode=…` with low-priority-first drop on narrow terminals via `fit_priority_segments`) |
| `views::roi` | Interactive tool rank with sort cycling + `recent_calls`. F1.11 unified table: work tools + user-blocking tools (e.g. `ask_user`) share one selectable table separated by a DIM divider; user-blocking rows are themselves DIM + marked with `*` in the # column (was a separate non-selectable sub-table pre-F1.11). New public helpers: `partition_and_sort(rows, sort_key)` returns `(work_sorted, blocking_sorted)`; `roi_selected_row(work, blocking, selected)` → `Option<&ToolRankRow>`; `is_selection_user_blocking(work, blocking, selected)` → `bool`. Detail strip below the table switches content based on selection kind: work → "recent 5 calls (turn(dur✓))"; user-blocking → "You waited N times totaling Xm Ys (not counted in agent work)". F1.12 — two new `%` columns surfaced: `OK%` (success rate, sister to the `s` sort key — `format_ok_pct(success, calls)` truncates rather than rounds so `99/100 → 99%`) and `Tot%` (this tool's share of session total tool duration — `format_total_pct(tool_total, total_all_ms)` shows `0%` for sub-1% slices, `—` only when total is 0); session total is computed once per render across all `tool_rank` rows. F1.13 — Tool cell color hints via `failure_severity_color(call_count, failure_count) -> Option<Color>`: Red+Bold for > 50% failure rate, Yellow+Bold for any failure on busy tools (>= 3 calls), default otherwise. Applies to both work and user-blocking sections; tints only the Tool cell so other columns stay legible on dark themes. F1.14 — user-blocking section anchored to the visible bottom of the table when content underfills (blank padding rows inserted between work and separator); when content overflows the visible area, padding is 0 and the layout reverts to F1.11 sequential ordering so viewport scrolling works as before. **F2.3** — Tool cell composes pending-state Yellow Bold via `compose_tool_cell_style(failure, is_pending) -> Style` (failure wins over pending per spec §3.3 — broken > slow); detail strip prefixes `⚠ N pending (<longest> longest) · ` when the selected tool has any pending calls (reuses the F2.1 `pending_calls` helper). |
| `views::aggregate` | By-Mode + By-Hook tables (single session) + `group_by_mode`; M1.6.3 adds a cross-session arm rendering `AnyAggregateReport` for `aggregate --export tui` and `watch aggregate ...`. F1.15 — By Mode adds `Turns%` and `Dur%` columns (reuses `views::roi::format_ok_pct` + `format_total_pct`). F1.16 — By Hook adds `OK%` column + `Hook` cell color hints via the shared `views::roi::failure_severity_color` (Red+Bold > 50% fail, Yellow+Bold any fail on ≥ 3 calls). F1.17 — By Mode `Out tokens` cell uses `views::models::format_token_u64_short` so large u64 sums abbreviate to 5-char `k/M/G/T/P` (saves ~5 cells per row). F1.18 — both tables fall back to a vertically-centered DIM placeholder when empty (`(no turns recorded ...)` / `(no hook events recorded ...)`) via the shared `render_empty_state` helper. F1.19 — By Hook block title is now `By Hook (single session — hook events)`, parallel to the By Mode title's `(single session)` qualifier |
...
| `views::mcp_waste` (M1.6.5, M1.6.6) | Split-pane MCP server-waste view (key `5`). M1.6.6 T5.1 — banner expands to **2 lines**: line 1 is `Source: <ds>   Loaded: <count>/<≈?><tokens> tokens   Unused: <count>/<≈?><tokens> tokens   Fully-unused servers: <n>` and line 2 is `Tokenizer: <kind>   Token source: <provenance>` (≈ prefix when `TokenProvenance::Heuristic` / `Mixed`); both server table and tool table gain a `Tokens` column (8 cells) showing `unused_tokens` / `description_tokens` respectively, formatted by the local `format_int_short` helper (`1.6K` style for `>= 1000`) with per-row ≈ prefix derived from `TokenSource::Heuristic`. Left 40% = server summary table (server / loaded-unused ratio / Tokens / `!` for fully-unused); right 60% = per-tool detail for the cursor-selected server (call_count, Tokens, source). `McpWasteState` holds the server cursor (`server_cursor: TableState`) + `unused_only: bool` filter; T5.3 dispatch routes `Up`/`Down`/`k`/`j` → cursor moves and `u` → filter toggle. Renders a "data not provided" fallback when constructed via `AppRunner::new` (e.g. `cmd::watch`, no waste pipeline). See `docs/superpowers/specs/2026-06-08-m1.6.5-mcp-waste-design.md` §7.4 + ADR-0015 D-4 |
...
| `u` (in McpWaste) | Toggle "unused tools only" filter on the right pane (M1.6.5) |
