...
| `views::mcp_waste` (M1.6.5) | Split-pane MCP server-waste view (key `5`). Top banner = data source + loaded/unused/fully-unused totals; left 40% = server summary table (server / loaded-unused ratio / `!` for fully-unused); right 60% = per-tool detail for the cursor-selected server (call_count, source, `unused` flag). `McpWasteState` holds the server cursor (`server_cursor: TableState`) + `unused_only: bool` filter; T5.3 dispatch routes `Up`/`Down`/`k`/`j` → cursor moves and `u` → filter toggle. Renders a "data not provided" fallback when constructed via `AppRunner::new` (e.g. `cmd::watch`, no waste pipeline). See `docs/superpowers/specs/2026-06-08-m1.6.5-mcp-waste-design.md` §7.4 + ADR-0015 D-4 |
