...
mod router_tests {
    ...
    fn seed_one_session(state: &AppState, id: &str) {
        use agentprof_core::adapter::AgentKind;
        use agentprof_core::analyzer::AnalysisReport;
        use agentprof_core::model::SessionMeta;
        use chrono::{TimeZone, Utc};

        // Pin to a fixed timestamp so snapshots are stable (the
        // normalization regex still strips it as a belt-and-braces
        // measure for the report's `Generated by ...` footer).
        let started = Utc
        ...
        agentprof_storage::upsert::upsert_report(
            &mut db,
            &report,
            std::path::Path::new("/tmp/agentprof-fixture.jsonl"),
            1_700_000_000,
        )
    ...
    #[tokio::test]
    async fn session_page_returns_404_for_unknown_id() {
        let (_tmp, state) = empty_db_state();
        let app = build_router(state);
        let req = Request::builder()
            .method(Method::GET)
    ...
    #[tokio::test]
    async fn api_session_chunk_returns_404_for_unknown_id() {
        let (_tmp, state) = empty_db_state();
        let app = build_router(state);
        let req = Request::builder()
            .method(Method::GET)
            .uri("/api/session/does-not-exist.html")
            .body(Body::empty())
            .expect("build req");
        let resp = app.oneshot(req).await.expect("oneshot");
        assert_eq!(resp.status(), StatusCode::NOT_FOUND);
    ...
    #[tokio::test]
    async fn session_page_returns_200_for_known_id() {
        let (_tmp, state) = empty_db_state();
        seed_one_session(&state, "abc12345-fixture");
        let app = build_router(state);
        let req = Request::builder()
            .method(Method::GET)
            .uri("/session/abc12345-fixture")
            .body(Body::empty())
            .expect("build req");
        let resp = app.oneshot(req).await.expect("oneshot");
        assert_eq!(resp.status(), StatusCode::OK);
    ...
    #[tokio::test]
    async fn api_session_chunk_omits_dashboard_chrome() {
        let (_tmp, state) = empty_db_state();
        seed_one_session(&state, "xyz98765-fixture");
        let app = build_router(state);
        let req = Request::builder()
            .method(Method::GET)
            .uri("/api/session/xyz98765-fixture.html")
            .body(Body::empty())
            .expect("build req");
        let resp = app.oneshot(req).await.expect("oneshot");
        assert_eq!(resp.status(), StatusCode::OK);
        ...
        assert!(!body.contains("<nav"), "chunk must not include nav");
        assert!(
            body.contains("<h1>Session"),
            "chunk must include session heading"
        );
    }

    #[tokio::test]
    async fn session_page_html_snapshot_fixture() {
        let (_tmp, state) = empty_db_state();
        seed_one_session(&state, "snap00000-fixture");
        let app = build_router(state);
        let req = Request::builder()
            .method(Method::GET)
            .uri("/session/snap00000-fixture")
            .body(Body::empty())
            .expect("build req");
        let resp = app.oneshot(req).await.expect("oneshot");
        let body = String::from_utf8(
        ...
        .expect("utf8");
        // Stabilize variable bits (version, Started-at, Generated-by
        // footer line) before snapshotting. Coarse line-replacement
        // avoids pulling `regex` into dev-deps.
        let normalized = body.replace(
            concat!("agentprof v", env!("CARGO_PKG_VERSION")),
            "agentprof v<VERSION>",
        );
        let normalized: String = normalized
            .lines()
            .map(|line| {
                if line.contains("Started:") || line.contains("Generated by") {
                    "<NORMALIZED>".to_owned()
                } else {
                    line.to_owned()
                }
